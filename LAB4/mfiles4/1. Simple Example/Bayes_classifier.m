...
% Computation of mean vector of training data for each class 
...
% Computation of covariance matrix of training data for each class. The
% ((N-1)/N) term allows for computing the the maximum-likelihood estimator
% of the Normal Distribution (biased sample variance), instead of the
% corrected variance (unbiased sample variance).
sigma_class1 = ((N-1)/N)*cov(class_1(1,:), class_1(2,:));
sigma_class2 = ((N-1)/N)*cov(class_2(1,:), class_2(2,:));
sigma_class3 = ((N-1)/N)*cov(class_3(1,:), class_3(2,:));

x1 = xtest(1,:);
x2 = xtest(2,:);

% Computation of the normal distribution probabilities for each class, for
% the Testing Data [x1; x2]
px_C1 = mvnpdf([x1(:) x2(:)], mean_class1, sigma_class1)';
px_C2 = mvnpdf([x1(:) x2(:)], mean_class2, sigma_class2)';
px_C3 = mvnpdf([x1(:) x2(:)], mean_class3, sigma_class3)';
